...
const Teacher = (props) => {
  ...
  const [screenRoom, setScreenRoom] = useState(null);
  const [videoRoom, setVideoRoom] = useState(null);


  const setScreen = (argRoom) => {
    setScreenRoom(argRoom);
  }
  const setVideo = (argRoom) => {
    setVideoRoom(argRoom);
  }
  ...
  const getTime = () => {
    return videoScreen.current.currentTime;
  }
  const url = {
    insertRoom : `${urlMain}api/main/updateroom/${classId}`,
    insertVideo : `${urlMain}api/main/insertVideo/${classId}`,
  ...
  useEffect(() => {
    ...
    if(roomData.screenRoom !== '' && roomData.videoRoom !== ''){
      const data = {
        roomNum : JSON.stringify(roomData),
      }
      axios.post(url.insertRoom, {data})
      .then((msg) => {
      }).catch((err) => {alert(err);});
  ...
  return (
    ...
    <div className="TeacherClass_Content">
      ...
      <div className="TeacherClass_SharingVideo" style={{}}>

        <div className="TeacherClass_Video" id="video_view">
          <JanusScreen state='create' video='screen' videoTag={videoScreen} setRoom={setScreen} playTime={playTime} classId={classId} className={className} />
        </div>
        <div className="TeacherClass_Video" id="video_view_Cam">
          <JanusScreen state='create' video='' videoTag={camScreen} setRoom={setVideo} playTime={playTime} classId={classId} className={className} />
        </div>
      </div>
    </div>
  )
